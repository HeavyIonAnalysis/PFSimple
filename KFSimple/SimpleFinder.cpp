...
float SimpleFinder::CalculateCosMomentumSum(const std::array<float, 8>& pars1, const std::array<float, 8>& pars2) {
  ...
  return (P1.at(0) * PSum.at(0) + P1.at(1) * PSum.at(1) + P1.at(2) * PSum.at(2)) / (std::sqrt(P1.at(0) * P1.at(0) + P1.at(1) * P1.at(1) + P1.at(2) * P1.at(2)) * std::sqrt(PSum.at(0) * PSum.at(0) + PSum.at(1) * PSum.at(1) + PSum.at(2) * PSum.at(2)));
}
...
float SimpleFinder::CalculateCosMomentumSumThird(const std::array<float, 8>& pars1, const std::array<float, 8>& pars2, const std::array<float, 8>& pars3) {
  ...
  return (P3.at(0) * PSum.at(0) + P3.at(1) * PSum.at(1) + P3.at(2) * PSum.at(2)) / (std::sqrt(P3.at(0) * P3.at(0) + P3.at(1) * P3.at(1) + P3.at(2) * P3.at(2)) * std::sqrt(PSum.at(0) * PSum.at(0) + PSum.at(1) * PSum.at(1) + PSum.at(2) * PSum.at(2)));
}
...
void SimpleFinder::FindParticles() {
  ...
  for (int iSecPos = 0; iSecPos < nSecPoses; iSecPos++) {
    for (int iSecNeg = 0; iSecNeg < nSecNegs; iSecNeg++) {
      ...
      if ((cuts_.GetIsApplyCutCosineDaughterPos() && (cos_pos < cuts_.GetCutCosineDaughterPos())) || (cuts_.GetIsApplyCutCosineDaughterNeg() && (cos_neg < cuts_.GetCutCosineDaughterNeg()))) continue;
