...
struct SelectionValues {
  ...
  float distance{std::numeric_limits<float>::max()}; ///< Distance between daughter tracks in their closest approach
  ...
  float cos_topo_{-1.f}; ///< Cosine of the angle between reconstructed mother's momentum and mother's radius vector beginning in the PV
