...
//      : distance_(distance), chi2_geo_(chi_2_geo), ldl_(ldl), chi2_topo_(chi_2_topo) {}
  ...
  void SetCutChi2GeoSM(std::vector<float> chi_2_geo_sm) { for (int i=1;i<chi_2_geo_sm.size()+1;i++) chi2_geo_.at(i) = chi_2_geo_sm.at(i-1); }
  ...
  void SetCutChi2TopoSM(std::vector<float> chi_2_topo_sm) { for (int i=1;i<chi_2_topo_sm.size()+1;i++) chi2_topo_.at(i) = chi_2_topo_sm.at(i-1); } 
  void SetCutCosTopo(float cos_topo) { cos_topo_.at(0) = cos_topo; }
  void SetCutCosTopoSM(std::vector<float> cos_topo_sm) { for (int i=1;i<cos_topo_sm.size()+1;i++) cos_topo_.at(i) = cos_topo_sm.at(i-1); }
