...
class Mother {
 ...
 public:
  ...
  void SetCutChi2GeoSM(std::vector<float> chi_2_geo_sm) {
    for (std::size_t i = 0; i < chi_2_geo_sm.size(); ++i) chi2_geo_.at(i) = chi_2_geo_sm.at(i);
  }
  ...
  void SetCutChi2TopoSM(std::vector<float> chi_2_topo_sm) {
    for (std::size_t i = 0; i < chi_2_topo_sm.size(); ++i) chi2_topo_.at(i) = chi_2_topo_sm.at(i);
  }
  void SetCutCosTopo(float cos_topo) { cos_topo_.at(0) = cos_topo; }
  void SetCutCosTopoSM(std::vector<float> cos_topo_sm) {
    for (std::size_t i = 0; i < cos_topo_sm.size(); ++i) cos_topo_.at(i) = cos_topo_sm.at(i);
  }
