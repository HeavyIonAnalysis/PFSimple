...
//   }
  ...
  int older_id = sim_track[mother_id_field_];
  while(older_id>=0)
  {
    const auto& simtrackolder = sim_tracks_[older_id];
    const int older_pdg = simtrackolder[sim_pdg_field_];
    ...
    if(ok == true) break;
    older_id = simtrackolder[mother_id_field_];
  }
